...
void Lab9::CreateStone(glm::vec3 position) {
...
void m1::Lab9::CreatePine(glm::vec3 position)
{
    glm::mat4 modelMatrix = glm::mat4(1);
    modelMatrix = glm::translate(modelMatrix, glm::vec3(position.x, position.y, position.z));
    ...
    modelMatrix = glm::scale(modelMatrix, glm::vec3(1.7, 0.5, 1.7));
    RenderSimpleMesh(meshes["cone"], shaders["LabShader"], modelMatrix, mapTextures["earth"], mapTextures["grass"]);

    modelMatrix = glm::translate(modelMatrix, glm::vec3(0, 0.8, 0));
    modelMatrix = glm::scale(modelMatrix, glm::vec3(0.8, 0.7, 0.8));
    RenderSimpleMesh(meshes["cone"], shaders["LabShader"], modelMatrix, mapTextures["earth"], mapTextures["grass"]);
}

void m1::Lab9::CreatePole(glm::vec3 position)
{
    glm::mat4 modelMatrix = glm::mat4(1);
    modelMatrix = glm::translate(modelMatrix, glm::vec3(position.x, position.y, position.z));
    modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1, 2, 0.1));
    RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, mapTextures["crate"]);

    modelMatrix = glm::mat4(1);
    modelMatrix = glm::translate(modelMatrix, glm::vec3(position.x, position.y + 1.1, position.z));
...
void m1::Lab9::CreatePlayer(glm::vec3 position)
{
    glm::mat4 modelMatrix = glm::mat4(1);
    modelMatrix = glm::translate(modelMatrix, glm::vec3(position.x, position.y, position.z));
    modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1, 0.01, 1));
    RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, mapTextures["crate"]);
    ...
    modelMatrix = glm::mat4(1);
    modelMatrix = glm::translate(modelMatrix, glm::vec3(position.x + 0.15, position.y + 0.3, position.z - 0.06));
    modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5));
    RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, mapTextures["crate"]);
}

void m1::Lab9::CreateGift(glm::vec3 position)
{
    glm::mat4 modelMatrix = glm::mat4(1);
    modelMatrix = glm::translate(modelMatrix, glm::vec3(position.x, position.y, position.z));
...
void m1::Lab9::CreateSnow(glm::vec3 position)
{
    glm::mat4 modelMatrix = glm::mat4(1);
    modelMatrix = glm::translate(modelMatrix, glm::vec3(position.x, position.y, position.z));
    modelMatrix = glm::scale(modelMatrix, glm::vec3(50, 0.01, 50));
...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    u +=  glm::vec3(0, 0, -2 * deltaTimeSeconds);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        glm::vec3 poz = glm::vec3(i * semn, 0.1, i % 7 * semn) + u;
        CreateStone(poz);

        poz = glm::vec3(i * -semn, 0.1, i % 9 * semn) + u;
        mix = 1;
        CreatePine(poz);
        mix = 0;
    }
    ...
    mix = 1;
    CreatePine(glm::vec3(-0.5, 0.3, 0) + u);
    mix = 0;

    CreatePlayer(glm::vec3(0, 0.1, 0));

    CreatePole(glm::vec3(1, 0.5, 0) + u);

    CreateGift(glm::vec3(1.5, 0.1, 0) + u);

    CreateSnow(glm::vec3(0));
