...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        mylightPosition = glm::vec3(0, 1.5, 1);
        lightDirection = glm::vec3(0, -1, 0);
        mylightDirection = glm::vec3(0, -1, 0);
...
void Lab8::Update(float deltaTimeSeconds)
{
    ...
    // TO DO: create another light source
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, mylightPosition);
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
        RenderMesh(meshes["sphere"], shaders["Simple"], modelMatrix);
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    light_position = glGetUniformLocation(shader->program, "my_light_position");
    glUniform3f(light_position, mylightPosition.x, mylightPosition.y, mylightPosition.z);

    light_direction = glGetUniformLocation(shader->program, "my_light_direction");
    glUniform3f(light_direction, mylightDirection.x, mylightDirection.y, mylightDirection.z);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_I)) {
            lightDirection -= glm::vec3(sin(ang), 0, 0) * deltaTime * speed;
            lightDirection += glm::vec3(0, cos(ang), 0) * deltaTime * speed;
        ...
        //if (window->KeyHold(GLFW_KEY_Y)) lightDirection -= glm::vec3(0, 0, 0.1) * deltaTime * speed;
        ////if (window->KeyHold(GLFW_KEY_Y)) lightDirection -= glm::vec3(0, 0.1, 0) * deltaTime * speed;
        //if (window->KeyHold(GLFW_KEY_U)) lightDirection += glm::vec3(0, 0, 0.1) * deltaTime * speed;
        ////if (window->KeyHold(GLFW_KEY_U)) lightDirection += glm::vec3(0, 0.1, 0) * deltaTime * speed;

        if (window->KeyHold(GLFW_KEY_Y)) {
            lightDirection -= glm::vec3(0, 0, sin(ang)) * deltaTime * speed;
            lightDirection += glm::vec3(0, cos(ang), 0) * deltaTime * speed;
            lightDirection = normalize(lightDirection);
        ...
        if (window->KeyHold(GLFW_KEY_K)) angle -= deltaTime * RADIANS(10);
        if (window->KeyHold(GLFW_KEY_L)) angle += deltaTime * RADIANS(10);
